...
async function readWriteStream(readableStream: NodeJS.ReadableStream) {
    return new Promise<string>((resolve, reject) => {
        ...
        //file write @filepath
        const writeFilePath = path.join(__dirname, sourceDirectoryFolderName, writeToFile);
...
async function downloadFileFromS3(g, c) {

    //user input
    let bucketName = g.req.body.bucketName; // s3 bucket name
    let downloadFilesList = g.req.body.targetFileWithPath; //  download filename with path

    let downloadFileArray = downloadFilesList.split(",");
    let listOfFileName: string;

    //download turn by turn
    for (let i = 0; i < downloadFileArray.length; i++) {

        //download file from a bucket
        let response = await c.send(new GetObjectCommand({
            Bucket: bucketName,
            Key: downloadFileArray[i]
        }));

        //get readable stream from response
        const readableStream = response.Body;

        // below will list all downloaded files with comma separate.
        if (listOfFileName) { listOfFileName = listOfFileName + ',' + await readWriteStream(readableStream); }
        else { listOfFileName = await readWriteStream(readableStream); }
    }
...
async function main(g: T.IAMGlobal) {
    ...
    try {
        ...
        let listOfFileName = await downloadFileFromS3(g, c);

        //convert all download files list into array
        let convertListIntoArray = listOfFileName.split(",");

        return {
            __am__downloadFileOrFolderPaths: convertListIntoArray, // save file into user system
            __am__downloadFolderFileName: overrideFileName || 'S3FileDownload.zip'  // take download file name as it is if override file name not exist
        };
    } catch (e) {
